...
public class SonarLintRuleBrowser extends Composite implements IPropertyChangeListener {
	...
	public SonarLintRuleBrowser(Composite parent, boolean useEditorFontSize) {
		...
		try {
			...
			// Browser is probably not available but it will be partially initialized in
			// parent
	...
	public void propertyChange(PropertyChangeEvent event) {
		...
		if (INFORMATION_BACKGROUND_COLOR.equals(event.getProperty())
				|| INFORMATION_FOREGROUND_COLOR.equals(event.getProperty())) {
	...
	private String css() {
		...
		return "<style type=\"text/css\">"
				+ "body { font-family: Helvetica Neue,Segoe UI,Helvetica,Arial,sans-serif; font-size: " + fontSizePt
				+ UNIT + "; " + "color: " + hexColor(this.foreground) + ";background-color: "
				+ hexColor(this.background) + ";}" + "h1 { margin-bottom: 0; font-size: 150% }"
				+ "h1 .rulename { font-weight: bold; }" + "h1 .rulekey { font-weight: normal; font-size: smaller;}"
				+ "h2 { font-size: 120% }" + "img { height: " + (fontSizePt + 1) + UNIT + "; width: " + (fontSizePt + 1)
				+ UNIT + "; vertical-align: middle; }"
				+ ".typeseverity span { font-size: 1em; margin-left: 0.5em; margin-right: 1em;}"
				+ "div.typeseverity { padding: 0; margin: 0}" + "a { border-bottom: 1px solid "
				+ hexColor(this.linkColor) + "; color: " + hexColor(this.linkColor)
				+ "; cursor: pointer; outline: none; text-decoration: none;}"
				+ "code { padding: .2em .45em; margin: 0; background-color: " + hexColor(this.foreground, 50)
				+ "; border-radius: 3px; white-space: nowrap;}" + "pre { padding: .7em; border-top: 1px solid "
				+ hexColor(this.foreground, 200) + "; border-bottom: 1px solid " + hexColor(this.foreground, 100)
				+ "; overflow: auto;}" + "code, pre { font-family: Consolas,Liberation Mono,Menlo,Courier,monospace;}"
				+ "ul { padding-left: 2.5em; list-style: disc;}" + "</style>";
	}
	...
	/**
	 * FIXME JFaceColors#getInformationViewerBackgroundColor was only introduced in
	 * Oxygen
	 */
	...
	/**
	 * FIXME JFaceColors#getInformationViewerForegroundColor was only introduced in
	 * Oxygen
	 */
	...
	public void updateRule(RuleDetails ruleDetails) {
		...
		if (ruleDetails == null) {
			browser.setText("<!doctype html><html><head>" + css()
					+ "</head><body><small><em>(Не выбрано ни одного правила)</em></small></body></html>");
		} else {
			...
			browser.setText("<!doctype html><html><head>" + css() + "</head><body><h1><span class=\"rulename\">"
					...
					+ "<div class=\"typeseverity\">" + "<img class=\"typeicon\" alt=\"" + type
					+ "\" src=\"data:image/gif;base64," + typeImg64 + "\">" + "<span>" + clean(type) + "</span>"
					+ "<img class=\"severityicon\" alt=\"" + severity + "\" src=\"data:image/gif;base64,"
					+ severityImg64 + "\">" + "<span>" + clean(severity) + "</span>" + "</div>"
					+ "<div class=\"rule-desc\">" + htmlDescription + "</div></body></html>");
		}
	}
	...
	private static String clean(String txt) {
		...
		return txt.toLowerCase(Locale.ENGLISH).replace("_", " ");
	}
